...
import { Button, Container, Dropdown, DropdownMenu, DropdownToggle, Navbar } from "react-bootstrap";
            ...
            <Navbar expand="md" className="nav-bg ">
                <Container>
                    ...
                    <Navbar.Collapse id="basic-navbar-nav">
                        <Nav fill className="navbar-container text-white justify-content-between">
                            {menu.map((item) => (
                                <Dropdown key={item.label} className="text-nav this-dropdown" >
                                    <DropdownToggle id="dropdown-autoclose-true" className="text-uppercase nav-buttons" >{item.label}</DropdownToggle>
                                    <DropdownMenu renderOnMount={true} rootCloseEvent="click" className="submenu" >
                                        {item.subMenu.map((subItem) => (
                                            <Dropdown.Item key={subItem.label} as={Link} to={subItem.link} className="text-nav" >{subItem.label} </Dropdown.Item>
                                        ))}
                                    </DropdownMenu>
                                </Dropdown>
                            ))}                        
